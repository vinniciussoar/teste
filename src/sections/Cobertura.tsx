...
export function Cobertura() {
  ...
  const map = useReveal<HTMLDivElement>(1);

  return (
    <section className="section cover" id="cobertura">
      <div className="container cover__grid">
        <div
          className={`cover__text${text.visible ? ' is-visible' : ''}`}
          ref={text.ref}
          data-reveal={text['data-reveal']}
        >
          <p className="eyebrow">Dónde atendemos</p>
          <h2 className="section-title">Atención en haras de toda Colombia</h2>
          ...
          <ul className="cover__list">
            <li><span className="cover__dot" aria-hidden="true" /> Haras de cría y caballos deportivos</li>
            <li><span className="cover__dot" aria-hidden="true" /> Desplazamiento con instrumental completo</li>
            <li><span className="cover__dot" aria-hidden="true" /> Urgencias coordinadas por WhatsApp</li>
          </ul>
          <a className="btn btn--primary" href={WA_LINK} target="_blank" rel="noopener">
            Consultar cobertura
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2}><path d="M5 12h14M13 6l6 6-6 6" /></svg>
          </a>
        </div>
        <figure
          className={`cover__map${map.visible ? ' is-visible' : ''}`}
          ref={map.ref}
          data-reveal={map['data-reveal']}
          data-delay={map['data-delay']}
          aria-label="Mapa de Colombia — atención en todo el país"
        >
          <svg viewBox="0 0 1024 1024" role="img" xmlns="http://www.w3.org/2000/svg">
            <title>Colombia</title>
            <g transform="translate(0.000000,1024.000000) scale(0.100000,-0.100000)">
              <path className="cover__map-shape" d="M5871 10207 c-10 -28 -18 -36 -46 -41 -21 -4 -32 -11 -29 -19 5 -15
...
-29 8 -36 18 -18 26 -32 32 -86 41 -26 5 -54 12 -63 15 -13 6 -20 -1 -28 -27z" />
            ...
            <circle className="cover__map-pin" cx={560} cy={560} r={13} />
            <circle className="cover__map-pulse" cx={560} cy={560} r={13} />
          </svg>
          <figcaption className="cover__map-cap">Cobertura nacional · Bogotá y alrededores</figcaption>
        </figure>
      </div>
    </section>
  );
